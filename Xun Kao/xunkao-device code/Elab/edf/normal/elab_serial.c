...
void elab_serial_isr_rx(elab_serial_t *serial, void *buffer, uint32_t size)
{
    ...
    {
        //if (elab_device_is_enabled(&serial->super))//isr需要尽快完成，不能使用互斥锁
        {
...
/**
  * @brief  Set the serial device mode.
  * @param  dev         The device handle.
  * @param  baudrate    elab_serial_set_mode.
  * @retval See elab_err_t.
  */
void elab_serial_set_mode(elab_device_t * const me, uint32_t mode)
{
    elab_serial_attr_t attr = elab_serial_get_attr(me);
    
    elab_assert(me != NULL);
    elab_assert(mode == ELAB_SERIAL_MODE_HALF_DUPLEX || mode == ELAB_SERIAL_MODE_FULL_DUPLEX);
